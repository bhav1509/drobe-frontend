...
class GarmentService {
  static Future<PagedGarmentResponse> getGarments({
    ...
    final response = await AuthService.get(
      Uri.parse('${ApiConfig.baseUrl}/api/garments?page=$page&size=$size'),
    );
  ...
  static Future<void> uploadGarment(File imageFile) async {
    final request = http.MultipartRequest(
      'POST',
      Uri.parse('${ApiConfig.baseUrl}/api/garments'),
    );
  ...
  static Future<void> deleteGarment(int garmentId) async {
    final response = await AuthService.delete(
      Uri.parse('${ApiConfig.baseUrl}/api/garments/$garmentId'),
    );
